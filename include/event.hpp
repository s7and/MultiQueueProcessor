...
public:
    std::pair<bool, Data> WaitForEvent( uint64_t timeout ) {
        ...
        bool status = _cv_eventUpdated.wait_for( _lk, std::chrono::nanoseconds(timeout), 
                                                  [this]{ return eventCame; });
        if( status && AutoReset ) eventCame = false;
...
public:
    bool WaitForEvent( uint64_t timeout ) {
        std::unique_lock<MutexType> _lk( _cv_lock );
        bool status = _cv_eventUpdated.wait_for( _lk, std::chrono::nanoseconds(timeout), [this]{ return eventCame; });
        if( status && AutoReset ) eventCame = false;
        return status;
    }
    void SetEvent() {
