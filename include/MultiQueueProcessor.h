...
template <typename Key, typename Value> struct IConsumer {
  virtual void Consume(const Key &, const Value &);
  virtual ~IConsumer(){};
};

template <typename Key, typename Value> struct IConsumer<Key, Value *> {
  virtual void Consume(const Key &, const Value *);
  virtual ~IConsumer(){};
};
...
public:
  Subscriber(const Key &key, const consumer_t consumer, const size_t bufferSize,
             const size_t threadCount)
      : BufferSize(bufferSize), ThreadCount(threadCount), key_(key),
        consumer_(consumer), bufferDivider(ThreadCount == 1 ? 2 : ThreadCount) {
...
template <typename Key, typename Value> class Queue {
  using consumer_t = IConsumer<Key, Value> *;
  using subscriber_t = Subscriber<Key, Value>;
  using subscriber_ptr = std::unique_ptr<subscriber_t>;

public:
  Queue(const size_t bufferSize = 1024, const size_t threadCount = 8,
        const size_t maxSubscribers = 64)
      : BufferSize(bufferSize), ThreadCount(threadCount),
  ...
  bool Subscribe(const Key &id, const consumer_t consumer) {
    ...
    if (MaxSubscribers <= subscribers.size())
      return false;
    auto [iter, ok] = subscribers.try_emplace(
        id,
        std::make_unique<subscriber_t>(id, consumer, BufferSize, ThreadCount));
    (void)iter;
    return ok;
  }
